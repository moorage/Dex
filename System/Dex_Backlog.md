...
- **Token Efficiency** (20%) - Does it reduce how much Codex needs to read? (Faster responses, lower costs)
- **Memory & Learning** (15%) - Does the system get smarter over time from this?
- **Proactivity** (10%) - Does this help Codex anticipate your needs?

**What "Codex Feasibility" Means:**
Ideas must work with what Codex can actually do - reading/writing files, running commands, and using MCP tools. Ideas that require tracking your edits in real-time or hooking into editor internals won't work, so they get scored 0.
...
- "Hey Codex, can you implement idea 001 please?"
- "I like idea 002, let's build it"
- "Use `$dex-improve` for idea 001"

The ideas below demonstrate the level of detail you'll see when you capture your own. Notice how each explains the problem, the solution, how it works in Codex, and the real-world benefit.
...
- **[idea-001]** Save meeting summaries so I don't re-read entire notes every time
  ...
  **The Problem:** 
  Right now, when you're preparing for a follow-up meeting, Codex has to re-read your entire meeting notes from scratch. If you had 3 meetings with Sarah last quarter, that's reading thousands of words just to remember "What did we decide last time?"
  ...
  **How It Works in Codex:**
  ✅ Totally doable! When you use `$process-meetings` or create a meeting note, Codex:
  ...
  **The Benefit:**
  Instead of reading 2,000 words of meeting notes, Codex reads 200 words of summaries. That's 90% less to process, which means faster responses and you stay under usage limits.
...
- **[idea-002]** Quick-reference directory of people so Codex doesn't read every person page

  **The Problem:**
  You have person pages for everyone you work with (managers, colleagues, customers). When you ask "Who's my contact at Acme Corp?" Codex currently has to open and read every person page file to find the answer. If you have 30 person pages, that's a lot of unnecessary reading.
  ...
  **How It Works in Codex:**
  ...
  1. When you create or update a person page, Codex quickly scans it
  2. Extracts just the basics: name, company, role, last interaction
  3. Adds or updates that one entry in the directory file
  4. When you ask "Who do I know at Acme Corp?", Codex checks the directory first (one small file)
  5. Only opens full person pages when you need detailed context
  
  **The Benefit:**
  Simple lookups become instant. Instead of reading 30 full person pages (15,000 words), Codex reads one directory file (500 words). That's 97% less to process for common questions.
...
*Use `$dex-backlog` to re-rank ideas based on current system state.* 
