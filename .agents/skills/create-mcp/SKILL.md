...
AI models like Codex are fundamentally **probabilistic** - they generate responses by predicting the most likely next token based on patterns in their training data. This is powerful for reasoning and language, but dangerous for facts:
...
Check if AGENTS.md has mcp_instructions section. If not, add:
...
✅ AGENTS.md updated with integration docs
...
3. Configure Codex to use the server:
   Edit `~/Library/Application Support/Codex/.mcp.json`:
   ```json
   {
     "mcpServers": {
       "[server-name]": {
...
Codex: [Shows education intro]

User: I want to connect my Todoist

Codex: Great choice! Let me understand what you need...
...
Codex: **Understood. Here's what we're building:**
...
Codex: Got it! Adding project filtering...
...
- Update ALL documentation (AGENTS.md, System Guide)
...
- [ ] AGENTS.md has integration documentation
...
See `docs/skill-analytics-checklist.md` for detailed guidance.
- [ ] User knows how to configure Codex to use the server
