...
1. List files in `.agents/skills/_available/[role_group]/`
...
1. **Verify skill exists** in `.agents/skills/_available/[role_group]/[skill-name]/`
2. **Copy skill folder:**
   ```bash
   cp -r .agents/skills/_available/[role_group]/[skill-name]/ .agents/skills/[skill-name]/
   ```
...
**Important:** Only copy the skill folder itself (not the parent role_group folder). The skill should end up at `.agents/skills/[skill-name]/SKILL.md`, not `.agents/skills/[role_group]/[skill-name]/SKILL.md`.
...
> - `AGENTS.md` for the full system overview
> - `.agents/skills/` for all available Dex workflows
