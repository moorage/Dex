...
Right now, Dex uses Codex for everything. It's excellent, but it costs money per use and requires internet.
...
**1. 💰 Budget Cloud Mode** — Cheaper AI when online
   ...
   **What it is:** Use models like Kimi or DeepSeek that cost 80-97% less than Codex
   ...
   **The catch:** Requires a small upfront payment (~$5-10) to a service called OpenRouter
   ...
   - Example: If Codex costs you $90/month, this would cost ~$10-20/month
...
**2. ✈️ Offline Mode** — AI that runs on your computer
   ...
   **The catch:** Uses your computer's memory and processing power
   ...
   - Slightly lower quality than Codex
...
- "Neither" — Stick with Codex for now
```
...
1. **Stick with Codex** — Best quality, you're already set up
2. **Budget cloud** — Much cheaper than Codex, but does need that initial payment
...
**My recommendation:** Start with **Kimi K2.5** — it's the closest to Codex in quality.

**Real cost example:**
If you currently spend ~$90/month on Codex, Kimi would cost ~$18/month for similar tasks.
...
**Option C: Set as default** (advanced)
   I can make Kimi your default and only use Codex for complex stuff
...
if ram <= 8:
    ...
    model = "qwen2.5:32b"
    quality = "Near-Codex for most tasks"
    size = "18 GB download"
else:
    model = "qwen2.5:72b"
    quality = "Excellent, close to Codex"
    size = "40 GB download"
```
...
What happens now:
- When you have internet → Dex uses Codex (best quality)
...
| Task Type | Model | Why |
|-----------|-------|-----|
| Complex planning, coaching | Codex | Needs deep reasoning |
...
| Task Type | Model | Why |
|-----------|-------|-----|
| Complex tasks | Codex | When you need the best |
| Simple tasks | Codex | (budget not configured) |
...
**Want to enable smart routing?**
- "Yes" — Codex when online, local when offline
- "No" — I'll always ask
```
...
**Want to enable smart routing?**
- "Yes" — Codex for complex, Kimi for simple
- "No" — I'll always ask
```
...
Smart routing works best when you have alternatives set up. Right now you only have Codex.

Would you like to set up:
- "Offline mode" — Free, works without internet
- "Budget cloud" — Cheaper models, needs ~$5-10 upfront
- "Neither for now" — Stick with Codex
```
...
Here's what I'll do:
- Complex tasks (planning, coaching) → Codex
- Simple tasks (summaries, quick questions) → Kimi (saves money)
- When offline → Your local model

You can always override by saying "use Codex for this" or "switch to budget".
```
...
Here's what I'll do:
- When online → Codex (best quality)
...
Here's what I'll do:
- Complex tasks → Codex (best quality)
...
3. **Anthropic Direct** — If you have Codex credits already, we can optimize usage
...
2. **Stick with Codex** — Best quality, works everywhere
...
| Mode | Status |
|------|--------|
| Premium (Codex) | ✅ Always available |
| Budget Cloud | ❌ Not configured |
| Offline Mode | ✅ Ready (Qwen 2.5) |

**How to use offline mode:**
- When you're online: Dex uses Codex (best quality)
...
**Want to enable smart routing?** (I'll use Kimi for simple tasks, Codex for complex ones)
- "Yes" — Enable automatic switching
- "No" — I'll ask before switching
...
**Want to enable smart routing?** 
I'll automatically:
- Use Codex for complex tasks (planning, coaching)
