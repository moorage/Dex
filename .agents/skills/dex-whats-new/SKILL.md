...
```
$dex-whats-new              # Review learnings + check for agent tooling updates
$dex-whats-new --full       # Include detailed explanations
$dex-whats-new --learnings  # Only review session learnings
$dex-whats-new --codex      # Only check agent tooling updates
```
...
- `--learnings` - Only review learnings (skip Codex check)
- `--codex` - Only check agent tooling updates (skip learnings)

---

## Process

### Step 0: Review Session Learnings (Unless --codex flag)
...
Use `WebSearch` to find the latest Codex changelog:
- Search: "Codex changelog 2026" or "OpenAI Codex releases"
- Primary source: OpenAI's official Codex documentation
...
### Step 4: Present Codex Findings

**If agent tooling updates found:**
...
1. [Improvement combining both]
   Uses: [Your pattern] + [New Codex feature]
...
**CRITICAL:** After presenting findings, you MUST update `System/codex-state.json` using the Write tool.
...
5. Write the updated JSON back to `System/codex-state.json`
...
Current Codex features to track:
...
> 
> Try again later, or check manually at `https://developers.openai.com/codex/cli`"
...
- Assume user knows Codex internals
