...
| `$xray dex` or `$xray architecture` | How Dex works: AGENTS.md, hooks, MCPs, skills |
...
- `AGENTS.md` (~X tokens) — My personality, your preferences, available tools
...
**Why these files?** Your AGENTS.md tells me where to look. It says [quote relevant 
...
| I read AGENTS.md first | **System prompt** loads before anything |
| I created files with tools | **Tools** let AI take action, not just talk |
| I followed your file conventions | **AGENTS.md instructions** define your structure |
...
- `$xray dex` — Full architecture: AGENTS.md, hooks, MCPs, skills, vault
...
**The fundamental problem:** Every time you start a new chat, Codex has amnesia. 
It doesn't remember yesterday's conversation. It doesn't know your name, your 
projects, or what you discussed last week.

This is true for ALL LLM-based AI — ChatGPT, Codex, Gemini, all of them.
...
- Codex's window: ~200,000 tokens (~150,000 words)
- Your AGENTS.md: ~4,500 tokens
...
the **system prompt**. For Dex, that's `AGENTS.md`.

```
┌─────────────────────────────────────────────────┐
│ AGENTS.md (loaded before you type anything)     │
├─────────────────────────────────────────────────┤
...
**The power move:** Everything in AGENTS.md influences EVERY response. 
...
Codex can't *do* things by default — it can only generate text. But with 
...
gives Codex new capabilities.

**Without tools:** Codex can only talk.
**With tools:** Codex can take action on your behalf.
...
1. Your message gets added to context window
                    ↓
2. Codex sees: system prompt + history + your message
                    ↓
3. Codex decides: should I use a tool?
...
**Key insight:** Each turn, Codex sees MORE context (the history grows). 
...
**Problem:** Codex forgets everything between sessions.  
...
├─────────────────────────────────────────────────┤
│ • AGENTS.md - Core context & personality        │
...
### Building Block 1: AGENTS.md (The Brain)

The system prompt that defines who Dex is and how it behaves.

**Location:** `/AGENTS.md` (root of your vault)
...
MCP = Model Context Protocol. It's how Codex gets superpowers.

Each MCP server gives Codex new abilities:
...
Hooks fire (inject context)
     ↓
AGENTS.md loads (personality + instructions)
     ↓
You type your message
     ↓
Codex reads relevant files (on-demand)
     ↓
Codex uses MCPs (if needed)
     ↓
...
Codex reads text, processes it, generates text, writes text.
...
#### Step 4: AGENTS.md Loads
```
The entire AGENTS.md file enters the context window:
     ↓
...
The AI already has:
• Your personality and preferences (AGENTS.md)
...
Codex already knows who you are, what you've been working on, and what 
tools are available.
...
### Level 1: Edit AGENTS.md (5 minutes)

**The simplest customization.** Add your own instructions:

1. Open `AGENTS.md` in any editor
