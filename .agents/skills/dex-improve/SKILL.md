...
2. **Review what's new** — See recent Codex changes and how they could improve your system
...
- "What new Codex features should I be using?"
...
2. **What's new?** — Review Codex changes since [last check: DATE]
...
.agents/skills/                      # Existing reusable behaviors and workflows
System/pillars.yaml                  # Strategic priorities
```

**External (when needed):**
- Search web for latest Codex capabilities
- Check the official Codex documentation at developers.openai.com
...
Map requirements to optimal Codex features:
...
Review recent Codex changes and suggest improvements.

### Process

1. **Read state file:** `System/codex-state.json`
   - If missing, this is first run — do a full scan instead

2. **Fetch current changelog:** Use WebSearch to find latest Codex changelog/releases
...
```
📢 Codex Updates (since [last check date])
...
Location: `System/codex-state.json`
...
Review all known Codex capabilities against your current system.

### Process

1. **Inventory current usage:**
   - Scan `.codex/` for runtime config and hooks
   - Check `.agents/skills/` for skills
...
2. **Load capability reference:**
   ...
   - Search web for comprehensive Codex feature list if needed
...
[Which Codex features to use and why. Include trade-offs considered.]
...
> 📢 Codex Updates (since 2026-01-15)
>
> === NEW CAPABILITIES ===
...
This isn't just requirements gathering—it's capability-aware design. The goal is to help you leverage Codex's full potential while building exactly what you need, nothing more.
