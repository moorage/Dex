...
>
> 1. **Manual** (recommended to start) — Run `$process-meetings` whenever you want. Uses Codex directly, no API key needed.
>
> 2. **Automatic** — Runs in the background every 30 minutes, even when Codex is closed. Requires an API key.
>
> Which do you prefer?"
...
1. **Create folder structure** (silently, no terminal output needed):
   ...
   - `03-Tasks/`
   - `02-Week_Priorities/`
2. **Update AGENTS.md** with their profile:
   - Name
   - Role
   - Company size
...
5. Do not create an extra vault config file. `System/user-profile.yaml`, `System/pillars.yaml`, and `AGENTS.md` are the canonical Dex setup artifacts.
...
- VS Code or Codex (where you're running Codex)
