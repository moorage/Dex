...
**Example:** Instead of Codex reading 50 career evidence files individually (expensive, slow), the Career MCP scans them all in milliseconds and returns structured stats: "8 competencies, 42 evidence files, Technical Depth: 2 examples (weak), Product Strategy: 8 examples (strong)." Codex then coaches based on structured insights.
...
**What it does:**  
Apple Calendar integration via AppleScript. Reads events, attendees, and meeting context without leaving Codex.
...
Codex receives structured data and coaches: "You're close to promotion readiness (67/100). Your Product Strategy evidence is strong, but Technical Depth needs more documentation. Let's capture 2-3 examples from your recent system design work..."
...
Run the relevant `*-setup` skill to configure integrations interactively, or add MCP servers manually to Dex's local `.mcp.json`.

See `.mcp.json.example` for a complete config with all built-in servers:
...
3. Update `AGENTS.md` and the system guide
...
Dex includes background automation that runs independently of the interactive Codex session, enabling the system to learn continuously.

### Codex Changelog Monitoring

**Script:** `.scripts/check-codex-changelog.cjs`  
**Frequency:** Every 6 hours (via Launch Agent)  
**Purpose:** Monitor OpenAI Codex release notes for new features relevant to Dex

**How it works:**
1. Reads `System/codex-state.json` to get last check date
2. Fetches OpenAI release notes via HTTPS
3. Detects new versions or updates since last check
4. If changes found:
   - Writes alert to `System/changelog-updates-pending.md`
   - Updates `codex-state.json` with latest version and check date
5. Session start hook displays prompt to run `/dex-whats-new`

**Manual testing:**
```bash
node .scripts/check-codex-changelog.cjs --force    # Force check
node .scripts/check-codex-changelog.cjs --dry-run  # Preview mode
```
